...
class HeteroGraphConvModel(nn.Module):
    def __init__(self, feature_dim1, feature_dim2, num_rels1, num_rels2, 
        ...
        self.register_buffer('kegg_go_buffer', torch.zeros(1, 311+4650, feature_dim1, device=device))
    ...
    def forward(self, graph1, graph2, herb_feature, herb_original_feat, p_targets, n_targets):
        ...
        # i_permuted = i_features_original.permute(0, 2, 1)  # (B, dim1, 2364)
        # i_mapped = torch.matmul(i_permuted, self.total_map)  # (B, dim1, 7854)
        # i_features = i_mapped.permute(0, 2, 1)  # (B, 7854, dim1), targets
        i_features = torch.einsum("bnd,nl->bld", i_features_original, self.total_map)  # (B, 7854, dim1)
        
        kegg_go_features = self.kegg_go_buffer.expand(herb_feature.shape[0], -1, -1)  # (B, 311+4650, dim1), pathways
        # combined_features = torch.cat([self.linear(i_features), kegg_go_features], dim=1)  # (B, 7854+311=8165, dim2)

        i_features = torch.cat([i_features, kegg_go_features], dim=1)

        i_features = i_features.reshape(-1, self.dim1)  # (B*7854, dim1)
        # c_features = combined_features.view(-1, self.dim2)  # (B*8165, dim2)
        ...
        for o_item in range(self.layer_o):  # outer loop
            ...
            # output_c_features = c_features['node']  # (B*8165, dim2)
            # output_c_features = output_c_features.view(attention_weights2.shape[0], -1, self.dim2)  # (B, 8165, dim2)
            ...
            # combined_features = torch.cat([self.linear(f_features_reshape), kegg_go_features], dim=1)  # (B, 7854+311=8165, dim2)
            # combined_features = combined_features.reshape(-1, self.dim2)  # (B*8165, dim2)
            # c_features['node'] = combined_features

        o_features = i_features['node']  # (B*7854, dim1)
        o_features = o_features.view(attention_weights1.shape[0], -1, self.dim1)  # (B, 7854, dim1)
        o_features = o_features[:, :7854, :]
